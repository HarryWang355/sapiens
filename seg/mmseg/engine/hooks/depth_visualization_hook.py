...
@HOOKS.register_module()
class GarmentDepthVisualizationHook(Hook):
    def __init__(self,
                 draw: bool = False,
                 interval: int = 50,
                 show: bool = False,
    ...
    def vis_depth_map(self, depth_map, mask=None, background_color=100, cond_depth_map=None, cond_mask=None):
        if mask is None:
            depth_map = (depth_map - depth_map.min()) / (depth_map.max() - depth_map.min())
            depth_map = 1 - depth_map ## 1 is near camera, 0 is far camera
            depth_map = (depth_map * 255.0).astype(np.uint8)
            depth_colored = cv2.applyColorMap(depth_map, cv2.COLORMAP_INFERNO)
            return depth_colored, None
